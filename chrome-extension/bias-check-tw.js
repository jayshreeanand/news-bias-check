...
chrome.storage.sync.get('biasCheckSettings', function(results){
  if(!results['biasCheckSettings']){
    ...
    showTwitter = true;
  }else if(results['biasCheckSettings']['twitter'] == undefined){
    // default to showing if Twitter is not set
    showTwitter = true;
  }else{
    showTwitter = results['biasCheckSettings']['twitter']
  }
  if(showTwitter){
    runBiasCheck();
  }
...
function runBiasCheck(){
  ...
  function run(sourceHash){
    ...
    var sourcesArray = Array.from(document.querySelectorAll(tweetClass + ':not(.bias-check)')).filter(function(e) { return e.querySelector(linkIcon) });
    
    // add bias-check class to each element to make sure that script isn't run multiple times on the same element
    sourcesArray.forEach(function(e){ e.classList.add('bias-check') });
